...
<div id="foot">
  <small>
    <p>Generated by SnipeOffice <a href="http://dev-www.snipeoffice.org/core/plain/solenv/bin/mkdocs.sh">Module Description Tool</a></p>
    <p>Last updated:
EOF
LANG= date >> $output
cat - >> $output <<EOF
...
for module_name in *; do
  if [ -d $module_name ]; then
	...
	if [ -f "$cur_file" ]; then
	  ...
	  text="${text} &nbsp; <a href=\"http://dev-www.snipeoffice.org/core/tree/${module_name}\">cgit</a>"
...
if [ ${#empty_modules[*]} -gt 0 ]; then
  ...
  for module_name in "${empty_modules[@]}"; do
    echo -e "<li><a href=\"http://dev-www.snipeoffice.org/core/tree/${module_name}\">${module_name}</a></li>\n" >> "$BASE_OUTPUT/index.html"
  done
  echo -e "</ul>\n" >> "$BASE_OUTPUT/index.html"
fi
